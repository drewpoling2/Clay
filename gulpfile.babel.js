...
gulp.task("scripts", () => {
  return merge(
    gulp.src(config.js_vendor_src).pipe($.size({ title: "Vendor scripts" })),
    gulp.src(config.js_build_src).pipe($.babel())
  )
    .pipe($.if(argv.pretty, $.sourcemaps.init()))
...
gulp.task("clean", () => {
  return del.sync([config.del_folder]);
});

// Copy the content from src/ to dist/
gulp.task("copy", ["clean"], () => {
  return gulp
    .src(config.src_folder)
...
gulp.task("serve", ["styles", "scripts"], () => {
  ...
  gulp.watch(
    ["./src/js/vendor/*.js", "./src/js/build/*.js"],
    ["scripts", reload]
  );
});
...
gulp.task("default", ["clean"], () => {
  ...
  runSequence("serve", () => {
    ...
    console.log("\x1b[32m  Clay is ready to move!\x1b[0m\n");
    console.log("\x1b[42m************************************\x1b[0m\n");
    console.log(
      "[\x1b[32mClay\x1b[0m]",
      `All finished in \x1b[35m${Date.now() - startTime} ms`,
      "\x1b[0m\n"
    );
  });
});
